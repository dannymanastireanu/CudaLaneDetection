...
//	global variables
bool draw_on_right = false;
bool draw_on_left = false;
...
__global__ void bitwiseAnd(unsigned char *image, unsigned char *mask, unsigned char *output_image, int width, int height) {

	int x = blockDim.x * blockIdx.x + threadIdx.x;
	int y = blockDim.y * blockIdx.y + threadIdx.y;

	if (x > 0 && y > 0 && x < width - 1 && y < height - 1) {
		output_image[y * width + x] = image[y * width + x] * mask[y * width + x];
	}
}
...
cudaError_t extractROI(unsigned char *image, unsigned char *roi, int width, int height)
{
	...
	Mat mask = Mat::zeros(Size(width, height), CV_8U);
	Point points[4] = {
	  Point(1013, 700),
	  Point(275, 700),
	  Point(490, 365),
	...
	cudaStatus = cudaMalloc(&d_mask, width * height * sizeof(unsigned char));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		goto Error;
	}

	cudaStatus = cudaMalloc(&d_roi, width * height * sizeof(unsigned char));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		goto Error;
	}

	cudaStatus = cudaMemset(d_roi, 0, width * height);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemset failed!");
		goto Error;
	}
	...
	cudaStatus = cudaMemcpy(d_mask, mask.data, width * height * sizeof(unsigned char), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy failed!");
		goto Error;
	}
	...
	bitwiseAnd << <numBlocks, threadsPerBlock>> > (d_image, d_mask, d_roi, width, height);

	// Check for any errors launching the kernel
	cudaStatus = cudaGetLastError();
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "bitwiseAnd launch failed: %s\n", cudaGetErrorString(cudaStatus));
		goto Error;
	}
	...
	cudaStatus = cudaDeviceSynchronize();
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaDeviceSynchronize returned error code %d after launching bitwiseAnd!\n", cudaStatus);
		goto Error;
	}

	cudaStatus = cudaMemcpy(roi, d_roi, width * height * sizeof(unsigned char), cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy failed!");
		goto Error;
	}
...
vector<Vec4i> lineClassification(vector<Vec4i> lines, double ts, bool on_right)
{
	vector<Vec4i> right_line, left_line, filtered_lines;
	vector<Point> right_points, left_points;
	Vec4d single_left_line, single_right_line;
	vector<double> slope_container;


	for (auto line : lines) {
		Point start_point = Point(line[0], line[1]);
		Point end_point = Point(line[2], line[3]);

		double slope = (double(end_point.y - start_point.y) / double(end_point.x - start_point.x));

		if (abs(slope) > ts) {
			if (slope > 0) {
				right_line.push_back(line);
				right_points.push_back(start_point);
				right_points.push_back(end_point);
				draw_on_right = true;
			} else {
				left_line.push_back(line);
				left_points.push_back(start_point);
				left_points.push_back(end_point);
				draw_on_left = true;
			}
		}
	}
	...
	if (right_points.size() > 0) {
		fitLine(right_points, single_right_line, DIST_L12, 0, 0.01, 0.01);
	}

	if (left_points.size() > 0) {
		fitLine(left_points, single_left_line, DIST_L12, 0, 0.01, 0.01);
	}
...
void plotLines(vector<Vec4i> lines, Mat image_rgb) {

	vector<Vec4i> right_line = lineClassification(lines, 0.5, true);
	vector<Vec4i> left_line = lineClassification(lines, 0.5, false);

	//	maybe i wiil draw later some line with this
	for (size_t i = 0; i < right_line.size(); i++)
	{
		Vec4i l = right_line[i];
		line(image_rgb, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(0, 255, 0), 3, LINE_AA);
	}
